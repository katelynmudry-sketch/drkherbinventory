...
export type InventoryLocation = 'backstock' | 'tincture' | 'clinic' | 'bulk' | 'bulk_backstock' | 'bulk_clinic';
    ...
    mutationFn: async (item: {
      ...
      if (item.location === 'backstock' || item.location === 'clinic') {
        const { data: activeBatch } = await supabase
          ...
          .select('id')
          .eq('user_id', user.id)
          .eq('herb_id', item.herb_id)
          .eq('status', 'active')
          .order('pressed_date', { ascending: false })
          .limit(1)
          .maybeSingle();
        current_batch_id = activeBatch?.id ?? null;
      }
